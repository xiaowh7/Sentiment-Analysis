...
def replaceHashtag(tweet, token):
    ...
    for i in range(len(tweet)):
        if token[i] == '#' or tweet[i].startswith('#'):
            count += 1
            token[i] = '#'
            tweet[i] = tweet[i][0:].strip(specialChar)
...
def removeProperCommonNoun(tweet, token):
    ...
    for i in range(len(tweet)):
        ...
        if token[i] != '^' and token[i] != 'Z':
            ...
            count[token[i]] += 1
    return newTweet, newToken
    # return tweet, token, [count['^'], count['Z']]
...
def removePreposition(tweet, token):
    ...
    for i in range(len(tweet)):
        if token[i] != 'P':
            ...
            countPreposition += 1
    return newTweet, newToken
    # return tweet, token, countPreposition
...
def preprocesingTweet1(tweet, token, emoticonsDict, acronymDict):
    ...
    tweet, token, countAcronym = expandAcronym(acronymDict, tweet, token)
    tweet, countRepetition = replaceRepetition(tweet)
    tweet, token, countHashtag = replaceHashtag(tweet, token)
    tweet, token, countURL = removeUrl(tweet, token)
    tweet, token, countTarget = removeTarget(tweet, token)
    tweet, token = expandNegation(tweet, token)
    return tweet, token, countAcronym, countRepetition, countHashtag, countURL, countTarget
...
def preprocesingTweet2(tweet, token, stopWords):
    ...
    return tweet, token, countNegation
